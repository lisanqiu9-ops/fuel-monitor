...
export const parseLedgerText = (source: string): ParsedLedger => {
  const text = source
    .replace(/^\uFEFF/, '')
    .replace(/\r\n?/g, '\n')
    .replace(/,\n/g, '\n')
    .replace(/,$/, '');
  const result = Papa.parse<RawLedgerRow>(text, {
  ...
  return { records, parserIssues, delimiter: result.meta.delimiter || ',', missingFields };
};

export async function parseLedgerFile(file: File): Promise<ParsedLedger> {
  return parseLedgerText(await file.text());
}
