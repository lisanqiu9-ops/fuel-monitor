...
export function LedgerTransactionsPage({ records }: { records: LedgerRecord[] }) {
  ...
  return <div>
    <div className="mb-4"><h2 className="text-lg font-black">流水明细</h2><p className="mt-1 text-xs font-bold text-[#8b7471]">搜索、筛选并核对每一笔记录</p></div>
    ...
    <div className="mt-4 flex items-center justify-between"><p className="text-xs font-black text-[#8b7471]">筛选结果</p><strong className="text-sm font-black text-[#76506f]">{filtered.length} 条</strong></div>
    <div className="mt-2 grid gap-2.5 md:hidden">{filtered.slice(0, 80).map(record => <div key={record.id + '-' + record.sourceRow}><LedgerTransactionCard record={record} /></div>)}{!filtered.length && <p className="rounded-2xl bg-white/60 p-6 text-center text-sm font-bold text-[#8b7471]">没有符合条件的流水。</p>}</div>
    <div className="mt-3 hidden md:block"><LedgerTable records={filtered} /></div>
  </div>;
}
